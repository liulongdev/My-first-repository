...
#define YYAnimatedImageView UIImageView
...
- (instancetype)init {
    ...
    _progressLayer.mar_size = CGSizeMake(40, 40);
...
- (void)layoutSubviews {
    ...
    _progressLayer.mar_center = CGPointMake(self.mar_width / 2, self.mar_height / 2);
}
...
//    [_imageView yy_cancelCurrentImageRequest];
    ...
    [_imageView mar_setImageDefaultCornerRadiusWithURL:item.largeImageURL];
    
    [self resizeSubviewSize];
}

- (void)resizeSubviewSize {
    _imageContainerView.mar_origin = CGPointZero;
    
    _imageContainerView.mar_width = self.mar_width;
    
    UIImage *image = _imageView.image;
    if (image.size.height / image.size.width > self.mar_height / self.mar_width) {
        _imageContainerView.mar_height = floor(image.size.height / (image.size.width / self.mar_width));
    } else {
        CGFloat height = image.size.height / image.size.width * self.mar_width;
        if (height < 1 || isnan(height)) height = self.mar_height;
        height = floor(height);
        _imageContainerView.mar_height = height;
        _imageContainerView.mar_centerY = self.mar_height / 2;
    }
    if (_imageContainerView.mar_height > self.mar_height && _imageContainerView.mar_height - self.mar_height <= 1) {
        _imageContainerView.mar_height = self.mar_height;
    }
    self.contentSize = CGSizeMake(self.mar_width, MAX(_imageContainerView.mar_height, self.mar_height));
    [self scrollRectToVisible:self.bounds animated:NO];
    
    if (_imageContainerView.mar_height <= self.mar_height) {
        self.alwaysBounceVertical = NO;
    } else {
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _scrollView.frame = CGRectMake(-kPadding / 2, 0, self.mar_width + kPadding, self.mar_height);
    ...
    _pager.mar_width = self.mar_width - 36;
    _pager.mar_height = 10;
    _pager.center = CGPointMake(self.mar_width / 2, self.mar_height - 18);
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    _snapshotImage = [_toContainerView mar_snapshotImageAfterScreenUpdates:NO];
    BOOL fromViewHidden = fromView.hidden;
    fromView.hidden = YES;
    _snapshorImageHideFromView = [_toContainerView mar_snapshotImage];
    fromView.hidden = fromViewHidden;
    
    _background.image = _snapshorImageHideFromView;
    if (_blurEffectBackground) {
        _blurBackground.image = [_snapshorImageHideFromView mar_imageByBlurDark]; //Same to UIBlurEffectStyleDark
    } else {
        _blurBackground.image = [UIImage mar_imageWithColor:[UIColor blackColor]];
    }
    
    self.mar_size = _toContainerView.mar_size;
    ...
    _scrollView.contentSize = CGSizeMake(_scrollView.mar_width * self.groupItems.count, _scrollView.mar_height);
    [_scrollView scrollRectToVisible:CGRectMake(_scrollView.mar_width * _pager.currentPage, 0, _scrollView.mar_width, _scrollView.mar_height) animated:NO];
...
#pragma clang diagnostic pop
    ...
    if (!item.thumbClippedToTop) {
        NSString *key = [[SDWebImageManager sharedManager] cacheKeyForURL:item.largeImageURL];

        if ([[SDWebImageManager sharedManager].imageCache imageFromDiskCacheForKey:key]) {
            cell.item = item;
        }
    }
    if (!cell.item) {
    ...
    if (item.thumbClippedToTop) {
        ...
        CGFloat scale = fromFrame.size.width / cell.imageContainerView.mar_width;
        
        cell.imageContainerView.mar_centerX = CGRectGetMidX(fromFrame);
        cell.imageContainerView.mar_height = fromFrame.size.height / scale;
        cell.imageContainerView.layer.mar_transformScale = scale;
        cell.imageContainerView.mar_centerY = CGRectGetMidY(fromFrame);
        ...
        [UIView animateWithDuration:oneTime delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            cell.imageContainerView.layer.mar_transformScale = 1;
        ...
        [UIView animateWithDuration:oneTime delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            cell.imageView.layer.mar_transformScale = 1.01;
        }completion:^(BOOL finished) {
            [UIView animateWithDuration:oneTime delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseInOut animations:^{
                cell.imageView.layer.mar_transformScale = 1.0;
                _pager.alpha = 1;
            }completion:^(BOOL finished) {
...
#pragma clang diagnostic pop
    ...
    if (fromView == nil) {
        ...
        [UIView animateWithDuration:animated ? 0.25 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
            self.alpha = 0.0;
            self.scrollView.layer.mar_transformScale = 0.95;
            self.scrollView.alpha = 0;
            self.pager.alpha = 0;
            self.blurBackground.alpha = 0;
        }completion:^(BOOL finished) {
            self.scrollView.layer.mar_transformScale = 1;
    ...
    if (_fromItemIndex != currentPage) {
        ...
        [_background.layer mar_addFadeAnimationWithDuration:0.25 curve:UIViewAnimationCurveEaseOut];
    } else {
        _background.image = _snapshorImageHideFromView;
    }
    ...
    [UIView animateWithDuration:animated ? 0.2 : 0 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (isFromImageClipped) {
            ...
            CGFloat scale = fromFrame.size.width / cell.imageContainerView.mar_width * cell.zoomScale;
            CGFloat height = fromFrame.size.height / fromFrame.size.width * cell.imageContainerView.mar_width;
            if (isnan(height)) height = cell.imageContainerView.mar_height;
            
            cell.imageContainerView.mar_height = height;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    CGFloat floatPage = _scrollView.contentOffset.x / _scrollView.mar_width;
    NSInteger page = _scrollView.contentOffset.x / _scrollView.mar_width + 0.5;
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            ...
            if (!cell) {
                ...
                cell.page = i;
                cell.mar_left = (self.mar_width + kPadding) * i + kPadding / 2;
...
- (void)updateCellsForReuse {
    for (YYPhotoGroupCell *cell in _cells) {
        if (cell.superview) {
            if (cell.mar_left > _scrollView.contentOffset.x + _scrollView.mar_width * 2||
                cell.mar_right < _scrollView.contentOffset.x - _scrollView.mar_width) {
...
- (void)showHUD:(NSString *)msg {
    ...
    hud.mar_size = CGSizeMake(label.mar_width + 20, label.mar_height + 20);
    hud.backgroundColor = [UIColor colorWithWhite:0.000 alpha:0.650];
    hud.clipsToBounds = YES;
    hud.layer.cornerRadius = 8;
    
    label.center = CGPointMake(hud.mar_width / 2, hud.mar_height / 2);
    [hud addSubview:label];
    
    hud.center = CGPointMake(self.mar_width / 2, self.mar_height / 2);
    hud.alpha = 0;
    [self addSubview:hud];
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    if (tile) {
        if (tile.zoomScale > 1) {
            ...
            CGFloat xsize = self.mar_width / newZoomScale;
            CGFloat ysize = self.mar_height / newZoomScale;
...
- (void)longPress {
    ...
    [UIView animateWithDuration:0.25 delay:0 usingSpringWithDamping:0.8 initialSpringVelocity:0.5 options:UIViewAnimationOptionLayoutSubviews animations:^{
        self.effectView.transform = CGAffineTransformMakeTranslation(0, -self.effectView.mar_height);
    } completion:^(BOOL finished) {}];
...
- (void)saveImage {
    ...
    UIImage *newImage = tile.imageView.image;
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                CGFloat vy = fabs(v.y);
                if (vy < 1) vy = 1;
                CGFloat duration = (moveToTop ? _scrollView.mar_bottom : self.mar_height - _scrollView.mar_top) / vy;
                duration *= 0.8;
                ...
                [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionBeginFromCurrentState animations:^{
                    ...
                    _pager.alpha = 0;
                    if (moveToTop) {
                        _scrollView.mar_bottom = 0;
                    } else {
                        _scrollView.mar_top = self.mar_height;
                    }
                } completion:^(BOOL finished) {
                    [self removeFromSuperview];
                }];
                ...
                _background.image = _snapshotImage;
                [_background.layer mar_addFadeAnimationWithDuration:0.3 curve:UIViewAnimationCurveEaseInOut];
                
            } else {
                [UIView animateWithDuration:0.4 delay:0 usingSpringWithDamping:0.9 initialSpringVelocity:v.y / 1000 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
                    _scrollView.mar_top = 0;
                    _blurBackground.alpha = 1;
                    _pager.alpha = 1;
                } completion:^(BOOL finished) {
