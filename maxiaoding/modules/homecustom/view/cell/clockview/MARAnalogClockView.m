...
#error MARAnalogClockView is built with Objective-C ARC. You must enable ARC for these files.
#endif

@interface MARAnalogClockView () {
...
@property (nonatomic, strong) MARHand *hourHand;

/// The minute hand. Subclass of UIView.
@property (nonatomic, strong) MARHand *minuteHand;

/// The second hand. Subclass of UIView.
@property (nonatomic, strong) MARHand *secondHand;

@property (nonatomic, strong) NSTimer *secondTimer;

@end

@implementation MARAnalogClockView
...
{
    if (!_secondTimer) {
        _secondTimer = [NSTimer timerWithTimeInterval:1.f target:[MARWeakProxy proxyWithTarget:self] selector:@selector(updateEverySecond) userInfo:nil repeats:YES];
        [[NSRunLoop mainRunLoop] addTimer:_secondTimer forMode:NSRunLoopCommonModes];
    }
    return _secondTimer;
}
...
- (void)layoutSubviews {
    if (shouldUpdateSubviews == YES) {
        ...
        self.hourHand = [[MARHand alloc] initWithFrame:self.bounds];
        ...
        self.minuteHand = [[MARHand alloc] initWithFrame:self.bounds];
        ...
        self.secondHand = [[MARHand alloc] initWithFrame:self.bounds];
...
- (void)addObservers
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_updateCurrentTime:) name:UIApplicationDidBecomeActiveNotification object:nil];
}
