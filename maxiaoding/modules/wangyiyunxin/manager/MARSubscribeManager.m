...
#import "MARSubscribeDefine.h"
#import <MARReachability.h>
#define MARSubscribeExpiry 60 * 60 * 24 * 1 //订阅有效期为 1 天

#define MARSubscribeEnable [NIMSDK sharedSDK].isUsingDemoAppKey //仅在使用 Demo App 的时候，将订阅能力开启，开发者可以根据自身应用订阅功能开启状态控制此开关。

NSString *const MARSubscribeNetState = @"net_state";

NSString *const MARSubscribeOnlineState = @"online_state";

@interface MARSubscribeManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
@implementation MARSubscribeManager

+ (instancetype)sharedManager
{
//    if (MARSubscribeEnable)
//    {
        static id instance;
...
- (void)start
{
    MARInfoLog(@"SubscribeManager Center Setup");
}
...
- (void)subscribeTempUserOnlineState:(NSString *)userId
{
    ...
    if (isRobot || isMe) {
        MARInfoLog(@"user can not subscribe temp publisher: %@",userId);
        //自己或者机器人，则不需要订阅
        return;
    }
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        MARInfoLog(@"subscribe temp publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
    }];
}
...
- (void)unsubscribeTempUserOnlineState:(NSString *)userId
{
    if (![_subscribeIds containsObject:userId])
    {
        ...
        [[NIMSDK sharedSDK].subscribeManager unSubscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
            MARInfoLog(@"unSubscribe temp publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
        }];
...
- (void)publishOnlineState
{
    ...
    event.value = MARCustomStateValueOnlineExt;
    event.sendToOnlineUsersOnly = NO;  //必须要让后登录的用户也能拿到
    NSDictionary *ext = @{
                          MARSubscribeNetState : @([MARReachability reachability].status),
                          MARSubscribeOnlineState : @(MAROnlineStateNormal), //移动端永远在线
                          };
    [event setExt:[ext mar_modelToJSONString]];
    [[NIMSDK sharedSDK].subscribeManager publishEvent:event completion:^(NSError * _Nullable error, NIMSubscribeEvent * _Nullable event) {
        MARInfoLog(@"publish online state error %@ ext %@ time %.2f",error,ext,event.timestamp);
    }];
}
...
- (void)subscribeNextMembers:(NSArray *)ids
{
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        MARInfoLog(@"subscribe publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (isMyFriend && ![self.subscribeIds containsObject:user.userId])
    {
        ...
        [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
            ...
            MARInfoLog(@"subscribe publisher: %@ error: %@",request.publishers,error);
        }];
    }
...
- (void)onRecvSubscribeEvents:(NSArray *)events
{
    ...
    for (NIMSubscribeEvent *event in events) {
        if ([self.subscribeIds containsObject:event.from] || [self.tempSubscribeIds containsObject:event.from])
        {
            ...
            if (oldEvent.timestamp > event.timestamp)
            {
                ...
                MARInfoLog(@"event id %@ from %@ is out of date, ingore...",event.eventId,event.from);
            }
            else
            {
                [eventsDict setObject:event forKey:event.from];
                MARInfoLog(@"receive event id %@ from %@ time %.2f",event.eventId,event.from,event.timestamp);
            }
    ...
    if (unSubscribeUsers.count)
    {
        ...
        [[NIMSDK sharedSDK].subscribeManager unSubscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
            MARInfoLog(@"unSubscribe publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
        }];
    }
}
...
- (NIMSubscribeRequest *)generateRequest
{
    ...
    request.expiry = MARSubscribeExpiry;
